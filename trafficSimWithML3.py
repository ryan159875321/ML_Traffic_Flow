...
pygame.display.set_caption("ML Traffic Lights - Optimised (60/90/120/180s Screengrabs)")
...
class TrafficLightController:
    ...
    def count_vehicles_near_intersections(self, vehicles):
        counts = {}
        for (ix, iy) in intersections:
            counts[(ix, iy)] = {'north': 0, 'south': 0, 'east': 0, 'west': 0}
            for v in vehicles:
                gx, gy = v.grid_pos
                if abs(gx - ix) <= 2 and abs(gy - iy) <= 2:
                    if v.direction == 'down' and gy < iy:
                        counts[(ix, iy)]['north'] += 1
                    elif v.direction == 'up' and gy > iy:
                        counts[(ix, iy)]['south'] += 1
                    elif v.direction == 'right' and gx < ix:
                        counts[(ix, iy)]['west'] += 1
                    elif v.direction == 'left' and gx > ix:
                        counts[(ix, iy)]['east'] += 1
        return counts

    def update(self, vehicles):
        self.timer += 1
        vehicle_counts = self.count_vehicles_near_intersections(vehicles)

        for (ix, iy), counts in vehicle_counts.items():
            total_NS = counts['north'] + counts['south']
            total_EW = counts['east'] + counts['west']

            
            if total_NS > total_EW + 2:
                #prioritse North/South traffic
                self.lights[(ix, iy)]['up'] = 'green'
                self.lights[(ix, iy)]['down'] = 'green'
                self.lights[(ix, iy)]['left'] = 'red'
                self.lights[(ix, iy)]['right'] = 'red'
            elif total_EW > total_NS + 2:
                #prioritise East/West traffic
                self.lights[(ix, iy)]['up'] = 'red'
                self.lights[(ix, iy)]['down'] = 'red'
                self.lights[(ix, iy)]['left'] = 'green'
                self.lights[(ix, iy)]['right'] = 'green'
            else:
                if self.in_all_red:
